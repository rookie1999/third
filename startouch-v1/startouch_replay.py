...
# 添加 interface_py 到路径（与本脚本同级目录下）
current_dir = Path(__file__).parent
interface_py_dir = current_dir / "interface_py"
...
def replay_trajectory(arm, trajectory, use_raw=True, speed_factor=1.0, skip_move_to_start=False, pos_tol=1e-3):
    """
    回放轨迹到机械臂
    ...
    # 移动到起始位置（如未预先对齐则执行）
    first_point = trajectory[0]
    if skip_move_to_start:
        print("跳过起始点移动（已预先到位）")
    else:
        print(f"移动到起始位置: {first_point['position']}")
        # 若当前已在起点附近，则无需重复移动
        try:
            current_pos, _ = arm.get_ee_pose_quat()
            if np.linalg.norm(current_pos - first_point['position']) < pos_tol:
                print("当前已在起始点附近，跳过移动")
            else:
                if use_raw:
...
def main():
    ...
    # 校验文件存在（trajectory_file 是字符串，需转换为 Path）
    ...
    try:
        ...
        print("机械臂初始化成功")
        # 初始化后立刻平移到指定偏移点，避免后续回放跳变
        try:
            _, init_euler_tmp = arm.get_ee_pose_euler()
            move_quat = euler_to_quaternion(init_euler_tmp[0], init_euler_tmp[1], init_euler_tmp[2])
        except Exception:
            move_quat = np.array([1.0, 0.0, 0.0, 0.0])
        offset_pos = np.array([0.335, 0.0, 0.175])
        arm.set_end_effector_pose_quat(pos=offset_pos, quat=move_quat, tf=3.0)
        time.sleep(3.5)
    except Exception as e:
    ...
    try:
        ...
        # 统一平移：将首点对齐到指定偏移位置，避免回弹
        offset_pos = np.array([0.335, 0.0, 0.175])
        delta = offset_pos - aligned_trajectory[0]['position']
        for i in range(len(aligned_trajectory)):
            aligned_trajectory[i]['position'] = aligned_trajectory[i]['position'] + delta
        #打印aligned_trajectory首点位置
        print(f"首点位置: {aligned_trajectory[0]['position'].tolist()}")
        print(f"首点已平移到 {offset_pos.tolist()}，后续轨迹同步平移")
        ...
        # 直接回放：第一帧就是偏移后的首点，避免重复回弹
        replay_trajectory(
            arm,
            aligned_trajectory,
            use_raw=use_raw,
            speed_factor=speed_factor,
            skip_move_to_start=True,  # 由回放函数统一发送首点
            pos_tol=1e-3
        )
