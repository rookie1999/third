...
def euler_to_rot6d(euler_angles, seq='xyz', degrees=False):
    ...
    Args:
        euler_angles (np.ndarray): 欧拉角 [roll, pitch, yaw]。
                                   形状可以是 (3,) 或 (N, 3)。
        seq (str): 旋转顺序，默认为 'xyz' (对应机械臂常用的 RPY)。
        degrees (bool): 输入是否为角度制。默认为 False (弧度)。

    Returns:
        np.ndarray: 6D 旋转向量 [r11, r21, r31, r12, r22, r32]。
                    如果输入是 (3,)，返回 (6,) -> 解决了你之前的报错问题
                    如果输入是 (N, 3)，返回 (N, 6)
    """
    # 1. 维度处理：记录原始形状是否为 1D
...
class SingleArm:
    ...
    def set_end_effector_pose_rot6d_raw(
        self,
        pos: Optional[Union[List[float], np.ndarray]] = None,  # Shape: (3,)
        rot: Optional[Union[List[float], np.ndarray]] = None,  # Shape: (3,)
    ) -> bool:
        """
        /**
        * @brief 设置末端执行器的目标位姿
        ...
        */
        """
        euler = euler_to_rot6d(rot)
        self.arm.set_end_effector_pose_raw(target_pos=pos, target_euler=euler)
        return True

    def get_end_effector_pose_rot6d(self):
        result = self.arm.get_end_effector_pose()
        pos_t = result[0]  # 类型np.ndarray
        rpy_t = result[1]
        rot = euler_to_rot6d(rpy_t)
        return pos_t, rot
