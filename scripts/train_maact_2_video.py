...
def main():
    ...
    # 根据模式调整 num_workers
    num_workers = 8 if args.video else 4

    # ==========================
    # 2. 数据集准备
    # ==========================
    # 无论哪种模式，都先扫描 hdf5 文件 (作为索引)
    ...
    if args.video:
        if VideoBasedEfficientMADataset is None:
            raise ImportError("efficient_ma_video_dataset.py is missing!")
    ...
    config = SpeedACTConfig(
        ...
        chunk_size=CHUNK_SIZE, n_obs_steps=N_OBS_STEPS,
        image_features={cam: (3, 480, 640) for cam in CAMERA_NAMES},
        main_camera=MAIN_CAMERA_NAME,
        robot_state_feature=(STATE_DIM,), action_feature=(ACTION_DIM,),
        use_optical_flow=True, object_detection_ckpt_path=YOLO_CKPT,
        cropped_flow_h=64, cropped_flow_w=64,
        feedforward_activation="relu", pre_norm=False
    )
