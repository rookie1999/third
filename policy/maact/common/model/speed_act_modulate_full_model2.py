...
from policy.maact.optical_flow2.gmflow_wrapper import GMFlowWrapper
...
class SpeedACT(nn.Module):
    def __init__(self, config: SpeedACTConfig):
        ...
        if self.config.use_vae:
            ...
            if self.config.use_optical_flow:
                self.optical_flow_model = GMFlowWrapper(checkpoint_path=self.config.gmflow_ckpt_path)

                self.optical_flow_encoder = OpticalFlowEncoder(
                    input_h=self.config.global_flow_size,
                    input_w=self.config.global_flow_size,
                ...
                self.fusion_module = DecoupledModulationModule(
                    ...
                    dropout=config.pre_fusion_dropout,
                )
    ...
    def forward(self, batch: dict[str, Tensor], return_features: bool = False) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        if self.config.use_optical_flow and "observation.images" in batch:
            ...
            with torch.no_grad():
                ...
                current_optical_flow_batch = self.optical_flow_model(resized_prev_images, resized_current_images)

            flow_input = torch.nn.functional.interpolate(
                current_optical_flow_batch,
                size=(self.config.global_flow_size, self.config.global_flow_size),
                mode='bilinear',
                align_corners=False
            )

            flow_tokens = self.optical_flow_encoder(flow_input)

            _is_flow_valid_batch_internal.fill_(True)
        ...
        if return_features:
            return actions, (mu, log_sigma_x2), speed_logits, _is_flow_valid_batch_internal, encoder_out
