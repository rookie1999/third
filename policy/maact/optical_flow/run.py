...
def get_model(device="cuda"):
    ...
    if netNetwork is None:
        netNetwork = Network().to(device)
        netNetwork.train(False)
        netNetwork.eval()
...
def generate_flo(tenOutput, output_path):
...
def generate_input(img1, img2):
    target_width = 320
    target_height = 256
    img1 = img1.resize((target_width, target_height), PIL.Image.LANCZOS)
    img2 = img2.resize((target_width, target_height), PIL.Image.LANCZOS)
    img1 = numpy.array(img1)
    img2 = numpy.array(img2)
    tenOne = torch.FloatTensor(img1.transpose(2, 0, 1).astype(numpy.float32) / 255.0).cuda()
    tenTwo = torch.FloatTensor(img2.transpose(2, 0, 1).astype(numpy.float32) / 255.0).cuda()
    return tenOne, tenTwo
##########################################################

if __name__ == '__main__':
    img1 = PIL.Image.open("/home/benson/.cache/huggingface/lerobot/benson-zhan/"
                          "so100_act_single_loc_multi_speed/images/observation.images.depth/"
                          "episode_000010/frame_000112.png").convert('RGB')
    img2 = PIL.Image.open("/home/benson/.cache/huggingface/lerobot/benson-zhan/"
                          "so100_act_single_loc_multi_speed/images/observation.images.depth/"
                          "episode_000010/frame_000113.png").convert('RGB')
    img3 = PIL.Image.open("/home/benson/.cache/huggingface/lerobot/benson-zhan/"
                          "so100_act_single_loc_multi_speed/images/observation.images.depth/"
                          "episode_000040/frame_000122.png").convert('RGB')
    img4 = PIL.Image.open("/home/benson/.cache/huggingface/lerobot/benson-zhan/"
                          "so100_act_single_loc_multi_speed/images/observation.images.depth/"
                          "episode_000040/frame_000123.png").convert('RGB')
    img5 = PIL.Image.open("/home/benson/.cache/huggingface/lerobot/benson-zhan/"
                          "so100_act_single_loc_multi_speed/images/observation.images.depth/"
                          "episode_000080/frame_000102.png").convert('RGB')
    img6 = PIL.Image.open("/home/benson/.cache/huggingface/lerobot/benson-zhan/"
                          "so100_act_single_loc_multi_speed/images/observation.images.depth/"
                          "episode_000080/frame_000103.png").convert('RGB')
    # target_width = 448
    # target_height = 320
    # tenOne, tenTwo = generate_input(img1, img2)
    ...
    generate_flo(tenOutput, "./out1.flo")

    tenOne, tenTwo = generate_input(img3, img4)
    tenOutput = estimate(tenOne, tenTwo).cpu()
    generate_flo(tenOutput, "./out2.flo")

    tenOne, tenTwo = generate_input(img5, img6)
    tenOutput = estimate(tenOne, tenTwo).cpu()
    generate_flo(tenOutput, "./out3.flo")
